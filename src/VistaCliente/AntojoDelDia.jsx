...
export default function AntojoDelDia({ antojo, onAgregar }) {
  ...
  const { producto, presentacion } = antojo;

  const agregar = () => {
    onAgregar({
      ...producto,
      precio: antojo.precio_con_descuento,
  ...
  return (
    <section ref={ref} className={`antojo-dia reveal ${visible ? 'reveal-visible' : ''}`}>
      ...
      <div className="antojo-dia-info">
        ...
        <h3>{producto.nombre}{presentacion ? ` (${presentacion.nombre})` : ''}</h3>
